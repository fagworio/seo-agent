...
const views = [["batches", "Lotes"], ["waiting", "Aguardando validação"], ["results", "Resultados"], ["todas", "Todas"], ["calib", "Calibração"]] as const;
const pageSize = 10;
...
function Improvements() {
  ...
  return <div className="space-y-5"><header><h1 className="text-xl font-semibold">Melhorias</h1><p className="mt-1 max-w-3xl text-sm text-[var(--muted)]">Acompanhe o que foi implementado, quando será revalidado e qual resultado foi realmente observado.</p></header>
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-6" aria-label="Resumo das melhorias" data-verdicts="multiaxial"><Summary label="Implementadas" value={all.length} /><Summary label="Aguardando dados" value={all.length - measured.length} /><Summary label="Tráfego em alta" value={all.filter((item) => ["traffic_up", "improved", "engagement_up"].includes(item.verdict ?? "")).length} /><Summary label="Visibilidade sem clique" value={all.filter((item) => item.verdict === "visibility_up").length} /><Summary label="Neutras" value={all.filter((item) => ["neutral", "no_change"].includes(item.verdict ?? "")).length} /><Summary label="Regressões" value={all.filter((item) => ["worsened", "regressed"].includes(item.verdict ?? "")).length} /></section>
...
function ImprovementDrawer({ item, close }: { item: Experiment; close: () => void }) {
  ...
  return <Drawer title="Detalhes da melhoria" onClose={close}><div className="flex justify-between gap-3"><div><StatusBadge item={item} /><h2 className="mt-2 text-lg font-semibold">{item.keyword || friendlyType(item.opportunity_type)}</h2><p className="mt-1 text-sm text-[var(--muted)]">{item.implemented_action}</p></div><Button size="sm" variant="ghost" onClick={close}>Fechar</Button></div>
    ...
    <section className="mt-5"><h3 className="font-semibold">Previsão</h3>{Object.keys(item.forecast).length ? <div className="mt-2 grid grid-cols-2 gap-2"><Metric label="CTR esperado" value={formatPercent(item.forecast.expected_ctr)} /><Metric label="Oportunidade de cliques" value={formatNumber(item.forecast.gap_clicks)} /><Metric label="Conservador" value={formatNumber(item.forecast.conservative_clicks)} /><Metric label="Cenário realista" value={formatNumber(item.forecast.realistic_clicks)} /></div> : <Missing text="Nenhuma projeção foi registrada para esta melhoria." />}<p className="mt-2 text-xs text-[var(--muted)]">Previsão é cenário, não resultado garantido.</p></section><section className="mt-5"><h3 className="font-semibold">Predito vs Observado</h3><div className="mt-2 rounded-md border border-[var(--border)] bg-[var(--surface-raised)] p-3 text-sm"><dl className="space-y-1"><Row label="Antes (posição)" value={formatDecimal(before.position)} /><Row label="Previsão de cliques" value={item.forecast?.gap_clicks != null ? `+${formatNumber(item.forecast.realistic_clicks ?? item.forecast.gap_clicks)}` : "—"} /><Row label="Depois 28d (posição)" value={formatDecimal(after.position)} /><Row label="Δ posição" value={deltaLabel(delta, "position")} /><Row label="Precisão" value={verdictLabel(item.verdict)} /></dl></div></section>
...
function verdictLabel(verdict: string | null | undefined) { const m: Record<string, string> = { improved: "Dentro do esperado", neutral: "Neutro", worsened: "Piorou", insufficient_data: "Dados insuficientes", traffic_up: "Tráfego em alta", visibility_up: "Visibilidade em alta (sem clique)", engagement_up: "Engajamento em alta", regressed: "Regressão", no_change: "Sem mudança", mixed: "Sinais misturados" }; return m[verdict ?? ""] ?? "—"; }
