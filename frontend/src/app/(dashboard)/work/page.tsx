...
function Workbox() {
  ...
  return <div className="space-y-4">
    ...
    <div className="overflow-x-auto rounded-[9px] border border-[var(--border)]"><table className="w-full text-sm"><thead className="bg-[var(--surface-raised)] text-left text-xs text-[var(--muted)]"><tr><th className="px-3 py-2">Decisão</th><th className="px-3 py-2">Evidência principal</th><th className="px-3 py-2">Classe</th><th className="px-3 py-2">Prioridade</th></tr></thead><tbody>{visible.map((item) => { const view = presentOpportunity(item); return <tr key={item.id} className="border-t border-[var(--border)] hover:bg-[var(--surface-raised)]"><td className="max-w-2xl px-3 py-2"><button onClick={() => setParam("item", item.id)} className="block max-w-full text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)]"><span className="block text-xs font-medium text-[var(--primary)]">{view.label}</span><span className="mt-0.5 block line-clamp-1 font-medium">{displayTitle(item)}</span><span className="mt-0.5 block line-clamp-1 text-xs font-normal text-[var(--muted)]">{view.detail}</span></button></td><td className="whitespace-nowrap px-3 py-2 text-xs text-[var(--muted)]">{opportunityEvidenceSummary(item)}</td><td className="px-3 py-2"><ActionBadge value={item.action_class} /></td><td className="px-3 py-2 tabular-nums">{formatPriority(item.score)}</td></tr>; })}{!items.length && <tr><td colSpan={4} className="px-3 py-6 text-center text-[var(--muted)]">Nenhuma decisão nesta visão.</td></tr>}</tbody></table><Pagination page={page} pageSize={pageSize} total={items.length} onPageChange={(next) => { setPage(next); setParam("page", String(next)); }} label="decisões" /></div>
